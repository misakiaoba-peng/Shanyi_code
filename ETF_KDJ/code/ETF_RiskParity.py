...
class ETF_RiskParity(object):
	...
	def strategy(self, **args):
		...
		"""
		self.period1 = args['period1']
		self.period2 = args['period2']
		self.weight = args['weight']
		# self.risk_exposure = args['risk_exposure']
		
		def Sharpe(data, period):
			ret = data.pct_change()
		...
		self.sharpe_l = Sharpe(self.close_buy_df, self.period2)
		self.indicator = self.weight * self.sharpe_s + (1-self.weight) * self.sharpe_l # 强弱指标
		self.indicator = self.indicator[self.indicator.index >= self.start_backtest]
		self.indicator = self.indicator.subtract(self.indicator.min(axis = 1), axis = 0) # 使指标变为正的
	...
	def performance(self):
		...
		plt.savefig(os.path.join(outdir, 
			f"total_asset_{self.period1}_{self.period2}_{self.weight}" + \
...
def select_params(nums):
	for d in os.listdir(output_path):
		...
		res = df_total.groupby(['Period1', 'Period2','weight']).size().reset_index(name = 'counts')
...
def run_parameters(c, args, outdir, output_excel):
	...
	if output_excel:
		with pd.ExcelWriter(os.path.join(
			...
			c.sharpe_s.to_excel(writer_excel, sheet_name = '短期Sharpe值')
			c.sharpe_l.to_excel(writer_excel, sheet_name = '长期Sharpe值')
...
def run(start: str, end: str, arg_mat:list, cycle:"int > 0" = 15, ETF_ls: list = select_ls, \
	...
	c = ETF_RiskParity(ETF_ls, future_ls, start, end, cycle)
	logger.info(f"Load data {start}-{end} finished")

	with open(os.path.join(outdir, f'summary_{start}_{end}.csv'), 'a', encoding='utf_8_sig', newline='') as csvfile:
		writer_csv = csv.writer(csvfile)
		writer_csv.writerow(['Period1', 'Period2', 'weight', \
			'累计收益率', 'Sharpe', '年化收益', '胜率', '盈亏比', '最大日收益率', '最大日亏损率', '最大回撤', 'MAR', '累计手续费'])

	l = Lock()
	pool = Pool(maxtasksperchild = 400, initializer  = init, initargs = (l, q))
	for row in arg_mat:
		if row[1] < row[0] or (row[0] == row[1] and row[2] != 0.1):
			continue
		args = {}
		args['period1'] = int(row[0])
		args['period2'] = int(row[1])
		args['weight'] = row[2]
...
if __name__ == '__main__':
	...
	arg_mat = list(itertools.product(period, period, weight))
	# ---------------------------------------跑Train的参数------------------------------------------------------------------
	...
	# ---------------------------------------DEBUG----------------------------------------------------------------
	run('2016.01.01', '2016.07.01', [[60, 85, 0.9]], 60, select_ls, [], True)
