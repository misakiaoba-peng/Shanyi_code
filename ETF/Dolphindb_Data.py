...
class GetData():
    ...
    def Future_hist_candle_spread(self,symbol1,c1,symbol2,c2,dateStart,dateEnd,ktype=1): #历史k线价差 不限期货
        sql=f"Get_History_Spread({dateStart},{dateEnd},`{symbol1},{c1},`{symbol2},{c2},{ktype})"
        return s.run(sql)

    def Future_hist_Tick_spread(self,symbol1,c1,symbol2,c2,dateStart,dateEnd): #历史tick价差，不限期货
        sql=f"Get_History_Spread_Tick({dateStart},{dateEnd},`{symbol1},{c1},`{symbol2},{c2})"
        return s.run(sql)

    def Indicators(self,product,dateStart,dateEnd,type='main',ktype='1'):
        df=self.Future_hist_Mcandle(product,dateStart,dateEnd,type,ktype)
    ...
    def Pattern(self,product,dateStart,dateEnd,type='main',ktype='1'):
        df = self.Future_hist_Mcandle(product, dateStart, dateEnd, type, ktype)
        df.rename(columns={'close': 'Close', 'open': 'Open', 'high': 'High', 'low': 'Low'}, inplace=True)
